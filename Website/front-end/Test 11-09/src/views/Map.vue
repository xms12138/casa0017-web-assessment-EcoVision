...
export default {
  ...
  setup() {
    ...
    // Map<normalizedBoroughName, { center:[lng,lat], bearing, bounds }>
    let busiestStreetByBorough = null;

    // 当前被“淡化”的 borough（用于高亮红线）
      ...
      try {
        ...
        map.on("click", BOROUGH_FILL_ID, (e) => {
          ...
          popup.on("close", () => {
            ...
            // 弹窗关闭时恢复填充透明度
            if (map && map.getLayer(BOROUGH_FILL_ID)) {
    ...
    // 加载 busiest_streets.geojson + 建 borough -> line 映射 + 画线
    ...
    // 飞到指定 borough 的 busiest street，同时淡化该区颜色
    const flyToBusiestStreet = (normBoroughName) => {
